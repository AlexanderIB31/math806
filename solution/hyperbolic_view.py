...
from solution.finite_difference.hyperbolic_pde import HyperbolicPDE
...
def unpack_params(rget):
    ...
    try:
        ...
        params['initial_args'] = [FunctionFromStr(rget[x]) for x in initial_args_names]
        ...
        params['init_orders'] = [x for x in init_orders_names if x in rget and rget[x] == 'on']
...
def validate_params(params, errors):
    ...
    if len(params['init_orders']) == 0:
        errors.append("Не выбран тип аппроксимации начальных условий")
        return False

    if a ** 2 <= 0:
        errors.append("Кэффициент 'a'^2 должен быть больше 0")
        return False
    ...
    psi_1(min_x)
    if not psi_1.valid:
        not_valid_funcs.append('psi_1(x)')
    psi_2(min_x)
    if not psi_2.valid:
        not_valid_funcs.append('psi_2(x)')
    if len(not_valid_funcs) != 0:
        errors.append("Функции заданы некорректно: " + ', '.join(not_valid_funcs))
        return False

    if 'explicit' in params['scheme_types']:
        sigma = a * step_t / step_x
        if sigma > 1:
            errors.append('Число Куранта (sigma) в явной схеме не может превосходить 1')
...
def hyperbolic(request):
    ...
    solver = HyperbolicPDE(a, b, c, -e, f,
                           alpha, beta, phi_0,
                           gamma, delta, phi_l,
                           psi_1, psi_2,
                           min_x, max_x,
    ...
    for scheme in params['scheme_types']:
        for bord_order in params['app_orders']:
            for init_order in params['init_orders']:
                try:
                    cur_solution = solver.solve(N, K, scheme, bord_order, init_order)
                except Exception as e:
                    print('*********************')
                    print('hyperbolic: exception while solving', scheme, bord_order, init_order)
                    print('Parameters:')
                    for k in params.keys():
                        print(k, ':', params[k])
                    print(e)
                    datasets['errors_msg'].append(
                        'Не удается решить: {0} ({1}, {2}), попробуйте поменять параметры решения'.format(
                            pretty_label(scheme),
                            pretty_label(bord_order),
                            pretty_label(init_order)))
                    continue

                plot_data = {'label': '{0} ({1}, {2})'.format(pretty_label(scheme),
                                                              pretty_label(bord_order),
                                                              pretty_label(init_order)),
                             'y': cur_solution}
                ...
                if analytic_data is not None:
                    datasets['data_errors'].append(
                        {'label': '{0} ({1})'.format(pretty_label(scheme),
                                                     pretty_label(bord_order),
                                                     pretty_label(init_order)),
